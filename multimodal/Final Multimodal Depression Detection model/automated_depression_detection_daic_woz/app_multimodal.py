...
from src.inference_multimodal import load_multimodal_artifacts, predict_multimodal_audio, transcribe_audio_with_whisper


st.set_page_config(page_title="Multimodal Depression Risk Detection", page_icon="🎙️", layout="wide")
...
def cached_load_multimodal_artifacts():
    return load_multimodal_artifacts()
...
st.caption("Audio CNN-BiLSTM-Attention + transcript NLP fusion. Research prototype only; not a medical diagnosis tool.")

with st.sidebar:
    st.header("Model Status")
    if model_path.exists():
        st.success("Multimodal model found")
    else:
        st.error("Multimodal model not trained yet")
    st.info("Train with: python -m src.train_multimodal\n\nEvaluate with: python -m src.evaluate_multimodal")
    st.divider()
...
with prediction_tab:
    ...
    with col_a:
        uploaded_file = st.file_uploader("Option 1: Upload audio", type=["wav", "mp3", "ogg", "flac", "m4a"])
    with col_b:
    ...
    use_whisper = st.checkbox("Auto-transcribe audio using Whisper if manual transcript is empty", value=True)

    if audio_source is not None:
        st.audio(audio_source, format="audio/wav")
        if not model_path.exists():
            st.error("Multimodal model file is missing. Run `python -m src.train_multimodal` first.")
        else:
            ...
            if transcript_text:
                ...
                st.warning(
                    ...
                    "Install `faster-whisper` or `openai-whisper` and `sentence-transformers` for automatic text transcriptions."
                )

            with st.spinner("Running multimodal audio + transcript model..."):
                cfg_loaded, model, scaler = cached_load_multimodal_artifacts()
                result = predict_multimodal_audio(audio_source, transcript_text, cfg_loaded, model, scaler)
            ...
            prob_df = pd.DataFrame(
                {
                    ...
                    "Probability (%)": [result["non_depressed_probability"] * 100, result["depressed_probability"] * 100],
                }
            )
            ...
            st.divider()
            st.subheader("Audio Branch Explainability")
        ...
        st.info("Upload or record audio. For best practical result, provide/correct the transcript too.")
...
with dashboard_tab:
    ...
    if not existing:
        st.warning("No multimodal plots found. Run `python -m src.train_multimodal` and `python -m src.evaluate_multimodal` first.")
    else:
    ...
    metric_files = sorted(reports_dir.glob("*_multimodal_metrics.json"))
    if metric_files:
...
with guide_tab:
    ...
    st.markdown(
        """
**Audio branch:** WAV audio → MFCC + delta features → CNN → BiLSTM → Attention → audio embedding.  
**Text branch:** transcript → SentenceTransformer embedding → dense layers → text embedding.  
**Fusion:** audio embedding + text embedding → dense layers → depression risk probability.
...
For best demo, record 20–30 seconds and type/correct the transcript in the text box.
        """
    )
